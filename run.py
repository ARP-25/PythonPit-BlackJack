...
class Card:
    ...
    Attributes:
    - symbol (str): The symbol of the card
    (e.g., Hearts, Diamonds, Clubs, Spades)
    - value (int): The value of the card.
    - logo (str): The logo or identifier of the card (e.g., Ace, King, Queen,
    numbers).
    ...
    def __init__(self, symbol, value, logo):
        ...
        Parameters:
        - symbol (str): The symbol of the card (e.g., Hearts, Diamonds, Clubs,
        Spades).
        - value (int): The value of the card.
        - logo (str): The logo or identifier of the card (e.g., Ace, King,
        Queen, numbers).
        """
        self.symbol = symbol
        self.value = value
...
class Deck:
    ...
    - print_cards(): Prints the number of cards in the deck and details of each
    card.
    ...
    def __init__(self):
        ...
        The deck is created with 52 cards, including all four symbols and
        various values.
    ...
    def print_cards(self):
        ...
        """
        print("Number of cards in the deck:", len(self.cards))
...
class Participant:
    ...
    Methods:
    - __init__(name): Initializes a Participant instance with the given name
    and an empty hand.
...
class Player(Participant):
    """
    Represents a player in the blackjack game, inheriting from the Participant
    class.
    ...
    Methods:
    - __init__(name, bet, credits): Initializes a Player instance with the
    given name, bet, and credits.
    ...
    def __init__(self, name, bet=0, credits=1000):
        ...
        - credits (int, optional): The initial credits of the player.
        Defaults to 1000.
        """
        self.name = name
        self.hand = []
        ...
        while True:
            try:
                ...
                if bet < 1 or bet > self.credits:
                    raise ValueError(
                        f"\nInvalid bet. Please enter a positive integer "
                        f"between 1 and {self.credits}.\n")

                self.bet = bet
                break
            except ValueError:
                print(f"\nInvalid input. Please enter a valid positive "
                      f"integer between 1 and {self.credits}.\n")
    ...
    def round_won(self):
        ...
        print(f"{self.name} has won {self.bet*2} $.\n{self.name} has now "
              f"{self.credits} $ left in the bank.")
...
class Dealer(Participant):
    """
    Represents the dealer in the blackjack game, inheriting from
    ...
    def show_hand(self, first_card_secret=True):
        ...
        Parameters:
        - first_card_secret (bool, optional): Whether to keep the first card
        secret or not. Defaults to True.
        """
        if first_card_secret is True:
...
def ask_player_draw_card(player, deck):
    ...
    This function asks the player if they want to draw another card during
    their turn and prompts for a 'yes' or 'no' response. If the player chooses
    to draw a card ('yes'),the function adds a card to the player's hand from
    the deck.
    ...
    while True:
        try:
            draw = input(
                "Do you want to draw another card? (yes/no): ").lower()
...
def show_winner(player, dealer):
    """
    Determine and display the winner of the round based on the participants
    hands.

    This function evaluates the hands of the player and dealer to determine
    ...
    if dealer.hand_value() > 21:
        ...
        player.round_lost()
            ...
            and not (player.hand_value() > 21)):
        print(f"Player: {player.name} won!\n")
        player.round_won()
    elif (dealer.hand_value() > player.hand_value()
            and not (dealer.hand_value() > 21)):
        print(f"Dealer: {dealer.name} won!\n")
        player.round_lost()
    elif dealer.hand_value() == player.hand_value():
        print("It's a draw!\n")
...
def show_final_hands(player, dealer):
    ...
    This function prints the final hands of both the player and the dealer,
    along with their respective hand values. It displays the results of the
    round, showing the winner or indicating a draw.
...
def start_round(player, dealer, deck):
    ...
    This function initiates a round by shuffling the deck, prompting the player
    to place a bet, dealing initial cards to both the player and dealer,
    allowing the player to draw additional cards, enabling the dealer to draw
    cards as per the rules, showing the final hands, determining the winner,
    and updating the player's credits.
...
def continue_play_option(player):
    ...
    This function checks the player's remaining credits and prompts them to
    choose whether they want to play another round or leave the table based
    on their credits.
    ...
    Returns:
    bool: True if the player chooses to continue playing, False if they choose
    to leave.
    """
    if player.credits > 0:
        print(
            f"\nDo you want to play another round or leave the table with "
            f"{player.credits} $.\nType play to play or a random character to "
            f"exit the game.")
        player_choice = input()
        ...
        print("You got no money left to play. Start a new game to start ")
        print("again with 1000$.")
        return False


def validate_player_name(name):
    """
    Validate the player's name to ensure it contains only letters and is at
...
def run_game():
    ...
    This function manages the flow of a round of the blackjack game, including
    initializing the game, allowing the player to place a bet, dealing cards
    to the player and dealer, managing the player's turn, managing the
    dealer's turn, determining the winner of the round, and updating the
    player's credits accordingly.

    The function also allows the player to continue playing more rounds or
    exit the game.
    ...
    while True:
        ...
        if first_round:
            print("\nWelcome to the Black Jack Table."
                  " You're entering with a total credit of 1000 $.\n")
            while True:
                print(
                    "\nUnder which name do you want to be referred to? Please"
                    " enter below:")
                try:
                    player_name = input()
                    if validate_player_name(player_name):
                        ...
                        raise ValueError(
                            ...
                            " with only letters and at most 20 characters.")
                except ValueError as e:
                    print(str(e))
